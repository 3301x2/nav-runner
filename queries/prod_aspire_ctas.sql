...
--
-- HOW to run:
--   1. Open BQ Studio: https://console.cloud.google.com/bigquery?project=fmn-production-462014
...
CREATE SCHEMA IF NOT EXISTS `fmn-production-462014.staging`
OPTIONS (
...
CREATE OR REPLACE TABLE `fmn-production-462014.staging.aspire_primelife_meta_audience`
AS
SELECT *
...
SELECT
    ...
    (SELECT COUNT(*) FROM `fmn-production-462014.staging.aspire_primelife_meta_audience`)  AS prod_row_count,
    (SELECT COUNT(*) FROM `fmn-sandbox.staging.aspire_primelife_meta_audience`)
      - (SELECT COUNT(*) FROM `fmn-production-462014.staging.aspire_primelife_meta_audience`)
                                                                                            AS delta;
...
FROM `fmn-production-462014.staging.aspire_primelife_meta_audience`;
